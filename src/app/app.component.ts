...
export class AppComponent {
  ...
  cutImg() {
    ...
    const canvas = $('#imgCanvas')[0];
    const ctx = canvas.getContext('2d');
    let widthX = this.mouse.x - this.mouse.startX;
    let heihgtY = this.mouse.y - this.mouse.startY;
    let imgData = ctx.getImageData(this.mouse.startX, this.mouse.startY, widthX, heihgtY);
 ...
 initDraw(canvas) {
  ...
  saveTxt() {
    ...
    for (let i = 0; i < inputsArr.length; i++) {
      ...
      ctx.fillText(element.value, pos.left, (pos.top + (element.size / 2)));
      element.remove();
    }
